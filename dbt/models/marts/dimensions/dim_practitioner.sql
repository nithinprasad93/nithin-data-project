...
with practitioners as (
    select * from {{ ref('stg_sp_practitioners') }}
),

employees as (
    select * from {{ ref('stg_eh_employees') }}
),
...
ndis_screening as (
    ...
    from {{ ref('stg_eh_certifications') }}
...
final as (
    select
        {{ dbt_utils.generate_surrogate_key(['p.practitioner_id']) }} as practitioner_key,
        p.practitioner_id,
        p.eh_employee_id,
